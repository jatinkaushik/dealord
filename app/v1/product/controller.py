...
def feature_func(json_data): 
    try:
        feature_model = Category_Feature(name=json_data['name'], features_datatype_id=json_data['feature_type'], category_id=json_data['category_id'], units=json_data['units']) 
...
def delete_category_features(current_user, json_data):
    try:
        if check_current_user_category_id(current_user, json_data['sub_category_id']):
            category_feature_search = Category_Feature.query.filter_by(sub_category_id=json_data['sub_category_id']).first()
...
def edit_category_features(current_user, json_data):
    try:
        if check_current_user_category_id(current_user, json_data['id']):
            category_feature_search = Category_Feature.query.filter_by(id=json_data['id']).first()
            if not category_feature_search:
                return "category_feature_not_found"
...
def fetch_category_features(current_user, json_data):
    try:
        if check_current_user_category_id(current_user, json_data['sub_category_id']):
            check_type = Category_Feature.query.filter_by(category_id=json_data['category_id']).first()
            cat_features = {
                "features": []
            }
            for i in check_type:
...
def add_product(json_data):
    for i in json_data['product']:
        try:
            addproduct = Products(name = i['name'],category_id = i['categoy_id'])
